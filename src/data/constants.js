export const Bio = {
    name: "Harshvardhan Verma",
    roles: [
        "Full Stack Developer",
        "C/C++ Programmer",
    ],
    description: "Enthusiastic IT student with a strong foundation in programming, data structures and algorithms and a keen interest in Web Development. I'm seeking a Full Stack Web Developer or SDE position to apply my technical skills and contribute to yield high level results. Eager to learn, adapt, and excel in a dynamic software development environment.",
    github: "https://github.com/Harshvardhan-Verma",
    resume: "https://drive.google.com/file/d/1T9zEKRWX2j64BQpNnZggYk6qVYcjLjoA/view?usp=drive_link",
    linkedin: "https://www.linkedin.com/in/harshvardhan-v-609bb7200/",
    insta: "https://www.instagram.com/harshvv._/",
};

export const skills = [{
        title: "Frontend",
        skills: [{
                name: "React Js",
                image: "data:image/svg+xml;base64,",
            },
            {
                name: "Redux",
                image: "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
            },
            {
                name: "HTML",
                image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
            },
            {
                name: "CSS",
                image: "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
            },
            {
                name: "JavaScript",
                image: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
            },
            {
                name: "Bootstrap",
                image: "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
            },
        ],
    },
    {
        title: "Backend",
        skills: [{
                name: "Node Js",
                image: "https://nodejs.org/static/images/logo.svg",
            },
            {
                name: "Express Js",
                image: "data:image/png;base64,",
            },
            {
                name: "MySQL",
                image: "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
            },
            {
                name: "MongoDB",
                image: "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
            },
        ],
    },
    {
        title: "Others",
        skills: [{
                name: "Git",
                image: "https://camo.githubusercontent.com/fbfcb9e3dc648adc93bef37c718db16c52f617ad055a26de6dc3c21865c3321d/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f6769742d73636d2f6769742d73636d2d69636f6e2e737667",
            },
            {
                name: "GitHub",
                image: "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
            },
            {
                name: "Docker",
                image: "https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg",
            },
            {
                name: "VS Code",
                image: "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
            },

        ],
    },
];

export const experiences = [{
        id: 0,

        role: "Frontend Engineer Intern",
        company: "The Sparks Foundation",
        date: "June 2022 - July 2022",
        desc: " Developed and maintained user-facing webpages using HTML, CSS, JavaScript and jQuery and conducted thorough frontend testing by creating a dummy database ",
        skills: [
            "HTML",
            "CSS",
            "JavaScript",
            "Bootstrap",
            "jQuery"
        ],
        img: "Sparks.png",
    },

];

export const education = [{
        id: 0,
        img: "Indian_Institute_of_Information_Technology,_Sonepat_logo (1).png",
        school: "Indian Institute of Information Technology Sonepat",
        date: "Nov 2020 - June 2024",
        grade: "8.71 CGPA",
        desc: "I am currently pursuing a Bachelor's degree in Information Technology at Indian Institute of Information Technology Sonepat, Bhubaneswar. I have a CGPA of 8.71 as of now. I have taken courses in Data Structures and Algorithms, Object-Oriented Programming, Database Management Systems, Operating Systems, and Web Development.",
        degree: "Bachelor of Technology - BTech, Information Technology ",
    },
    {
        id: 1,
        img: "Schoollogo.jpg",
        school: "Mahatma Hansraj Modern School , Jhansi",
        date: "Apr 2017 - Apr 2019",
        desc: "I completed my class 12 high school education at Mahatma Hansraj Modern School, Jhansi, where I studied PCM with Physical Education",
        degree: "CBSE(XII), PCM",
    },
    {
        id: 2,
        img: "Schoollogo.jpg",
        school: "Mahatma Hansraj Modern School , Jhansi",
        date: "Apr 2016 - Apr 2017",
        desc: "I completed my class 10 education at Mahatma Hansraj Modern School , Jhansi where I studied Science",
        degree: "CBSE(X), Science",
    },
];

export const projects = [{
        id: 9,
        title: "Flappy Birds",
        description: "Utilized HTML, CSS, and Bootstrap to create visually appealing frontend designs.•   Leveraged JavaScript and JQuery to enhance user interactivity and implement card flipping and swapping  functionality.•Designed and developed an engaging Card Matching Game  for children to enjoy during their leisure time, contributing to a previous employer’ s entertainment offerings. ",
        image: "Flappybirds.png",
        tags: [
            "HTML",
            "CSS",
            "JavaScript",
            "jQuery",
            "BootStrap",
            "Github",
            "VS Code",
        ],
        category: "web app",
        github: "https://github.com/rishavchanda/Trackify",
    },

    {
        id: 1,
        title: "Basic Cricket Game",
        description: " Developed a fully functional 2D cricket simulation game using C++ from scratch.•Implemented a comprehensive user interface allowing players to select teams, control players, and track game progress ",
        image: "https://user-images.githubusercontent.com/64485885/234916413-96296f13-fe4b-4cc4-b215-e72bd7c27928.png",
        tags: [
            "C++",
        ],
        category: "web app",
        github: "https://github.com/rishavchanda/Project-Management-App",
    },
    {
        id: 2,
        title: "Brain Tumor Detection",
        description: "Preprocessed and augmented the dataset to improve model accuracy, trained the model, created API using model and Python, and used React web app for the project's front end. Achievements: Achieved an accuracy of 99.2% to accurately detect brain tumors from medical images.",
        image: "https://github.com/rishavchanda/Brain-Tumor-Detection/raw/main/Readme_resource/Image2.png",
        tags: ["Python", "Keras", "TensorFlow", "VGG16", "Pickle", "React"],
        category: "machine learning",
        github: "https://github.com/rishavchanda/Brain-Tumor-Detection",
        webapp: "https://brain-tumor.netlify.app/",
        member: [{
                name: "Rishav Chanda",
                img: "https://avatars.githubusercontent.com/u/64485885?v=4",
                linkedin: "https://www.linkedin.com/in/rishav-chanda-b89a791b3/",
                github: "https://github.com/rishavchanda/",
            },

        ],
    },
    {
        id: 6,
        title: "Expense Management Website",
        date: "Jun 2021",
        description: "A simple react app that shows the characters of the famous TV series Breaking Bad. It uses the Breaking Bad API to fetch the data. It also has a search bar to search for a particular character.",
        image: "https://camo.githubusercontent.",
        tags: ["React Js", "API", "Axios", "Node JS"],
        category: "web app",
        github: "https://github.com/rishavchanda/Breaking-Bad",
        webapp: "https://breaking-bad-webapp.netlify.app",
    },

];

export const TimeLineData = [
    { year: 2017, text: "Started my journey" },
    { year: 2018, text: "Worked as a freelance developer" },
    { year: 2019, text: "Founded JavaScript Mastery" },
    { year: 2020, text: "Shared my projects with the world" },
    { year: 2021, text: "Started my own platform" },
];